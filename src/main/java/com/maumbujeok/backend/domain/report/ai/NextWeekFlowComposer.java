...
public class NextWeekFlowComposer {
    public NextWeekFlowAiResponse compose(String name, String gender, String calendarType, String birthDate, String birthTime, String weeklyInsight) {
        String weeklySummaryText = weeklyInsight != null ? weeklyInsight : "이번 주 기록 없음";
        String displayName = name != null && !name.isBlank() ? name : "마음";
        String title = String.format("%s님을 위한 다음 주 마음 흐름", displayName);
        String adviceText = String.format(
        ...
        return new NextWeekFlowAiResponse(title, adviceText);
    }
}
