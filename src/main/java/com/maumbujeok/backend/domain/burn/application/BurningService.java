...
public class BurningService {
    ...
    public CreateBurningResponse create(String phone, CreateBurningRequest request) {
        ...
        if (request.sourceType() == BurningSourceType.DIARY) {
            diary = diaryRepository.findByIdAndMemberPhoneNumber(request.diaryId(), phone)
                    .orElseThrow(() -> error(ErrorCode.BURNING_NOT_FOUND, "Diary not found"));
            if (diary.isBurned() || burningRepository.existsByMemberPhoneNumberAndDiaryId(phone, diary.getId())) {
                throw error(ErrorCode.DIARY_ALREADY_BURNED, "Diary has already been burned");
            }
            diaryId = diary.getId();
        ...
        if (diary != null) {
            diary.markBurned(burning.getId(), now);
    ...
    @Transactional
    public TalismanCreationResponse createTalisman(String phone, Long burningId) {
        Burning burning = owned(phone, burningId);
        BurningAnalysis analysis = analysisRepository.findByBurningIdForUpdate(burningId)
                .orElseThrow(() -> error(ErrorCode.BURNING_NOT_FOUND, "Burning analysis not found"));
        if (talismanRepository.existsByBurnRitualId(burningId)) {
            throw error(ErrorCode.TALISMAN_ALREADY_EXISTS, "Talisman has already been created");
        }
        if (analysis.getStatus() != BurningAnalysisStatus.COMPLETED
                && analysis.getStatus() != BurningAnalysisStatus.FALLBACK_COMPLETED) {
            throw error(ErrorCode.BURNING_ANALYSIS_NOT_COMPLETED, "Burning analysis has not completed");
        }
        Talisman talisman = talismanRepository.saveAndFlush(Talisman.builder()
                ...
                .build());
        return new TalismanCreationResponse(true, TalismanItemResponse.from(talisman));
    }
    ...
    public BurningListResponse getPage(String phone, Long cursor, Integer requestedSize) {
        ...
        if (size < 1 || size > MAX_SIZE) throw error(ErrorCode.INVALID_BURNING_REQUEST, "size must be between 1 and 50");
        List<Burning> fetched = burningRepository.findCursor(phone, cursor, PageRequest.of(0, size + 1));
    ...
    @Transactional(readOnly = true)
    public BurningDetailResponse get(String phone, Long id) {
    ...
    private BurningRequestException error(ErrorCode code, String message) { return new BurningRequestException(code, message); }
}